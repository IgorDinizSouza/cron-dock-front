...
export default function CompradoresPage() {
  ...
  const [items, setItems] = useState<CompradorResponse[]>([])
  ...
  const load = async () => {
    try {
      setLoading(true)
      setItems(await compradorApi.listAll())
    } catch (error: any) {
      toast({
        title: "Erro",
        description: error?.message || "Não foi possível carregar os compradores.",
        variant: "destructive",
      })
    } finally {
      setLoading(false)
  ...
  const handleDelete = async (item: CompradorResponse) => {
    if (!confirm(`Deseja realmente excluir o comprador "${item.descricao}"?`)) return
    if (!confirm("Deseja realmente excluir o registro?")) return

    try {
      await compradorApi.delete(item.id)
      toast({ title: "Sucesso", description: "Comprador excluído com sucesso." })
      await load()
    } catch (error: any) {
      toast({
        title: "Erro",
        description: error?.message || "Não foi possível excluir o comprador.",
        variant: "destructive",
      })
    }
  }
    ...
    <div className="space-y-6">
      <div className="flex flex-col items-start justify-between gap-4 sm:flex-row">
        <div>
          ...
          <p className="text-gray-600">Cadastro e gerenciamento de compradores por grupo empresarial</p>
        </div>
        <div />
      </div>
      ...
      <Card>
        <CardHeader>
          <CardTitle>Compradores ({filtered.length})</CardTitle>
        </CardHeader>
        <CardContent>
          {loading ? (
            <div className="py-10 text-center text-gray-500">Carregando compradores...</div>
          ) : filtered.length === 0 ? (
            <div className="py-10 text-center text-gray-500">Nenhum comprador encontrado.</div>
          ) : (
            <div className="space-y-3">
              {filtered.map((item) => (
                <div key={item.id} className="flex items-center justify-between gap-4 rounded-lg border p-4 hover:bg-gray-50">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <div className="flex items-center gap-2">
                        ...
                        <Badge className={item.status === "INATIVO" ? "border-red-200 bg-red-100 text-red-800 hover:bg-red-100" : "border-green-200 bg-green-100 text-green-800 hover:bg-green-100"}>
                  ...
                  <div className="flex items-center gap-2">
                    <Link href={`/compradores/${item.id}/editar`}>
