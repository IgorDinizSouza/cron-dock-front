...
import { ArrowLeft, Factory, Loader2, Save } from "lucide-react"
...
import { grupoFornecedorApi, type GrupoFornecedorStatus } from "@/lib/grupo-fornecedor"

const GRUPO_DESCRICAO_MAX = 100

export default function NovoGrupoFornecedorPage() {
  const router = useRouter()
  const { toast } = useToast()
  const [loading, setLoading] = useState(false)
  const [descricao, setDescricao] = useState("")
  const [status, setStatus] = useState<GrupoFornecedorStatus>("ATIVO")
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setLoading(true)
      const created = await grupoFornecedorApi.create({ descricao: descricaoTrim, status })
      toast({ title: "Sucesso", description: "Grupo de fornecedores criado com sucesso." })
      router.push(`/configuracoes/grupos/fornecedores/${created.id}/editar`)
    } catch (error: any) {
      toast({
        title: "Erro",
        description: error?.message || "Não foi possível criar o grupo de fornecedores.",
        variant: "destructive",
      })
    } finally {
      setLoading(false)
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-gray-900">Novo grupo de fornecedores</h1>
            <p className="text-gray-600">Cadastre um novo grupo de fornecedores</p>
          </div>
        </div>
      </div>
          ...
          <CardContent className="space-y-4">
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
              ...
              <div>
                ...
                <Select value={status} onValueChange={(value) => setStatus(value as GrupoFornecedorStatus)} disabled={loading}>
