...
import { grupoFilialApi, type GrupoFilialResponse } from "@/lib/grupo-filial"
import { Building, Eye, Plus, Search } from "lucide-react"

export default function GruposUnidadesPage() {
  const { toast } = useToast()
  const [items, setItems] = useState<GrupoFilialResponse[]>([])
  ...
  const load = async () => {
    try {
      setLoading(true)
      setItems(await grupoFilialApi.listAll())
    } catch (error: any) {
      toast({
        title: "Erro",
        description: error?.message || "Não foi possível carregar os grupos de unidades.",
        variant: "destructive",
      })
    } finally {
      setLoading(false)
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col items-start justify-between gap-4 sm:flex-row">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Grupos de unidades</h1>
          <p className="text-gray-600">Cadastro de grupos de filiais (unidades)</p>
        </div>
        <div />
      </div>
      ...
      <Card>
        <CardHeader className="space-y-3">
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div className="flex items-center gap-2">
              ...
              <Link href="/configuracoes/grupos/unidades/novo">
        ...
        <CardContent>
          <div className="text-sm text-gray-500">Filtre a tabela por descrição do grupo de unidades.</div>
        </CardContent>
      </Card>
      ...
      <Card>
        ...
        <CardContent>
          {loading ? (
            <div className="py-10 text-center text-gray-500">Carregando grupos de unidades...</div>
          ) : filtered.length === 0 ? (
                ...
                <tbody>
                  {filtered.map((item) => (
                    <tr key={item.id} className="border-b">
                      ...
                      <td className="px-3 py-3">
                        <Link href={`/configuracoes/grupos/unidades/${item.id}/editar`}>
